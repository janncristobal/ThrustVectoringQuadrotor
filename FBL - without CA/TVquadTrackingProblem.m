...
function status = myOutputFcn(t, x, flag)
    % Additional variable computation
    xdes = trajectory(t,x);
    %[K_pos1,~,K_att1,~] = controllerParams;
    %[F,M] = fbl(x,xd,K_pos1,K_att1);
    %v = [F;M];
    ...
    % Store additional variable in a persistent variable
    persistent additionalVars;

    if strcmp(flag, 'init')
        % Initialize the additional variable structure
        additionalVars = struct();
        additionalVars.xdes = [];
    elseif strcmp(flag, 'done')
        % Store the final additional variable values
        additionalVars.xdes = [additionalVars.xdes, xdes];
    else
        % Store the additional variable values during integration
        additionalVars.xdes = [additionalVars.xdes, xdes];
    end
